...
**為何 `idle` 也算孤兒**:`idle` 只代表「該 session 上一輪結束了」,它的**子程序仍然活著**(等下一個 prompt)。core 一死就再也沒有人能接手它,所以 `idle` 同樣是孤兒紀錄,只是它沒有「做到一半」的工作。

> ⚠️ **2026-09-04 更正**:這裡原本接著寫「core 一死,子程序也沒了」——**事實相反**。子程序沒有被綁進任何 OS 層級的連坐回收單位,core 被 SIGKILL 時它們會繼續活著。對帳流程本身不受影響(它處理的是 DB 紀錄),但「子程序已經沒了」這個前提不能拿來推論別的事。OS 上的孤兒**行程**由 `packages/adapters/src/child-registry.ts` 在下次啟動時回收,見該檔案頂端說明。
